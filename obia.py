"""
Created on Mon May 25 17:28:46 2020
@author: Clare
...
from skimage.segmentation import quickshift  # other version available, suited to application
import time

# Reading in Files
mt_stack_ds = gdal.Open('C:\\Users\\clare\\MscDiss\\Malham\\clipS2A\\mt_stack.tif')  # default read only
segments_fn = 'C:\\Users\\clare\\MscDiss\\Malham\\clipS2A\\mt_segments.tif'
...
# quickshift
seg_start = time.time()  # timing
segments = quickshift(img, kernel_size=1, convert2lab=False)  # 2d array with segment id

print('quickshift segments complete in ...', time.time() - seg_start, 'seconds')  # print time
seg_start = time.time()

# slic
# segments = slic(img,n_segments = 8000, compactness = 0.01)
# print('slic segments complete...', time.time()-seg_start) # print time

# save seg to raster
segments_ds = driverTiff.Create(segments_fn, mt_stack_ds.RasterXSize, mt_stack_ds.RasterYSize,
                                1, gdal.GDT_Float32)
segments_ds.SetGeoTransform(mt_stack_ds.GetGeoTransform())
segments_ds.SetProjection(mt_stack_ds.GetProjectionRef())
...
def segment_features(segment_pix):
    """
    calculate statistics for features
    """
    features = []
    npixels, nbands = segment_pix.shape
    for b in range(nbands):
        band_stats = stats.describe(segment_pix[:, b])
        if npixels == 1:
...
for id in segment_ids:
    segment_pixels = img[segments == id]  # where segemnts equals id, get image data
    # print('pixels for id', id, segment_pixels.shape)
    object_features = segment_features(segment_pixels)
    objects.append(object_features)
    object_ids.append(id)

print('create', len(objects), 'objects with', len(objects[0]), 'variables in', time.time() - obj_start, 'seconds')

# ------------------------------------#
# rasterising shapefile ground truth data points
# read in training dataset
train_fn = 'C:\\Users\\clare\\MscDiss\\\Malham\\clipS2A\\train.shp'
...
for klass in classes:
    ...
    segments_of_class = segments[ground_truth == klass]  # which segments correspond to each land cover type
...
train_img = np.copy(segments)  # np array copy
...
for klass in classes:
    # for each object, if  seg_id is in the segsegmetn of trianing data for that class, we'll get the value of that segment for training object
    ...
    # if we have 15 segmetns that represented wat, we woudl get list of water_id 15 times
    training_labels += [klass] * len(class_train_object)
    training_objects += class_train_object
    print('Training objects for class', klass, ':', len(class_train_object))
    # train objects shoudl be equal to traingin segments
...
# for each segment we want to list segment Id with rpeducted values
for segment_id, klass in zip(segment_ids, predicted):  # dictioary that links segment ids to predicted values
...
# ------------------------------#
# Rasterise output
clfds = driverTiff.Create('C:\\Users\\clare\\MscDiss\\\Malham\\clipS2A\\classified_img.tif',
